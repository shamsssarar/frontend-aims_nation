...
async function fetchApi<TData>(endpoint: string, options: ApiRequestOptions = {}): Promise<TData> {
  ...
  if (!response.ok) {
    ...
    console.error(`API Error [${response.status}] on ${endpoint}:`, jsonResponse.message);
    ...
    throw new Error(jsonResponse.message || 'An error occurred during the request');
  }
...
export const httpClient = {
  ...
  post: <TData>(endpoint: string, body?: unknown, options?: ApiRequestOptions) =>
    fetchApi<TData>(endpoint, { ...options, method: 'POST', body: JSON.stringify(body) }),

  put: <TData>(endpoint: string, body?: unknown, options?: ApiRequestOptions) =>
    fetchApi<TData>(endpoint, { ...options, method: 'PUT', body: JSON.stringify(body) }),

  patch: <TData>(endpoint: string, body?: unknown, options?: ApiRequestOptions) =>
    fetchApi<TData>(endpoint, { ...options, method: 'PATCH', body: JSON.stringify(body) }),
